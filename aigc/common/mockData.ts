...
export type UserMessage = {
	type?: 'right'; // 放页面右边的
	content?: string; // 用户回复的话
	images?: string[]; // 用户回复的图片
	interfaceParams?: Record< // 用于调用接口时计算
		string, // 用于调用接口时的参数名。对应技术方案文档里的各个字段
		string | // 参数值
		{ text: string, family: string } | 
		{ logo: string, QRCode: string, topicMap: string }>, 
	tooltipsBtns?: Array<'copy' | 'edit' | 'family'>;  // 对输入文案的操作按钮 -> 复制 ｜ 编辑 ｜ 字体
	nextRobotId?: number; // 用户回复后，机器人回复的那条消息
}
...
export type MsgItem = UserMessage | RobotMessage;
export type MsgList = MsgItem[];
// 如果是logo流程
export const robotReply_1 = {
	1: {
		data: {
			content: '请您简要描述您想要设计的logo的想法，\n例如：“一间开在海边的咖啡厅”\n请在下方输入框内进行描述并发送给我',
			canUserSend: true,
			afterUserSendNextRobotId: 2, // 等用户回复完，再下一次机器人
			nextUserReply: { // 下次用户回复时，会参考哪些参数
				interfaceParams: {
